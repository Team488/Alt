...
def convertLength(
    ...
    # "default case"
    if fromType == toType:
        return cast(Union[Tuple[float, ...], float, None], value)

    convertLengthFunc = lambda value: Measurements.Length.convert(
...
def convertRotation(
    ...
    Returns:
        Converted value(s), or None if conversion fails
    """
    convertRotFunc = lambda value: Measurements.Rotation.convert(
        value, fromType, toType
    )
...
def __convert(
    value: Union[Iterable[Any], Any], convertFunction: Callable[[Any], T]
