...
class AgentManager:
    def __init__(self):
        ...
        self.client = XTablesClient()
        self.propertyGenerator = PropertyOperator(XTablesClient(), ConfigOperator())

        self.allRunningAgents = self.propertyGenerator.createReadExistingNetworkValueProperty(UpdateOperator.ALLRUNNINGAGENTPATHS, self.client.getStringList(UpdateOperator.ALLRUNNINGAGENTPATHS))
        self.currentRunningAgents = self.propertyGenerator.createReadExistingNetworkValueProperty(UpdateOperator.CURRENTLYRUNNINGAGENTPATHS, self.client.getStringList(UpdateOperator.CURRENTLYRUNNINGAGENTPATHS))
    ...
    def getSubscription(self, agentBaseName : str) -> "AgentSubscription":
        ...
        newSubcrp = AgentSubscription(agentBaseName, self.propertyGenerator, self.client)
...
class AgentSubscription:
    ...
    TIMERSUBBASES = [AgentOperator.CREATETIMER, AgentOperator.PERIODICTIMER, AgentOperator.SHUTDOWNTIMER, AgentOperator.CLOSETIMER]

    def __init__(self, agentBaseName : str, propertyGenerator : PropertyOperator, client : XTablesClient) -> None:
        self.timerSubs = {}
        for timerSub in self.TIMERSUBBASES:
            table = f"{agentBaseName}.{self.TIMERBASE}.{timerSub}_Ms:"
            self.timerSubs[timerSub] = self._getSub(propertyGenerator, table, client.getDouble, -1)
        
        self.statusSub = self._getSub(propertyGenerator, f"{agentBaseName}.{AgentOperator.STATUS}", client.getString, "...")
        self.descriptionSub = self._getSub(propertyGenerator, f"{agentBaseName}.{AgentOperator.DESCRIPTION}", client.getString,  "...")
        self.errorsSub = self._getSub(propertyGenerator, f"{agentBaseName}.{AgentOperator.ERRORS}", client.getString, "none...")
    ...
    @staticmethod
    def _getSub(propertyGenerator : PropertyOperator, table : str, defaultGetter, default : str = None):
        networkDefault = defaultGetter(table)

        if networkDefault is None:
            networkDefault = default

        return propertyGenerator.createReadExistingNetworkValueProperty(table, networkDefault)
