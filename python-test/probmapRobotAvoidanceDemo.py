...
fieldMap = probmap.ProbMap(fieldX, fieldY, res,objSize,objSize,robotSizeX,robotSizeY,maxSpeedRobots=maxDist1Sec) #Width x Height at 1 cm resolution
...
def check_and_get_collision_coords(robotStartX, robotStartY, robotGoalX, robotGoalY, 
                                   obstacleStartX, obstacleStartY, obstacleGoalX, obstacleGoalY) -> tuple[int,int]:
    ...
    if obstacleStartX == obstacleGoalX:
        ...
        obstacle_slope = (obstacleGoalY - obstacleStartY) / (obstacleGoalX - obstacleStartX)
    ...
    if abs(robot_slope - obstacle_slope) < deltaM:
        ...
        if (min(robotStartX, robotGoalX) <= x <= max(robotStartX, robotGoalX) and
            min(robotStartY, robotGoalY) <= y <= max(robotStartY, robotGoalY)):
...
def getNextMove(cx,cy,goalX,goalY,time) -> tuple[int,int]:
    ...
    for prediction in preds:
        ...
        collision = check_and_get_collision_coords(cx,cy,cx+dx,cy+dy,curPx,curPy,newPx,newPy)
...
def run():
    ...
    while True:
        for i in range(20000):
            ...
            global colisionsDetected
            cv2.putText(mergedMap,f"Collisions Pred:{colisionsDetected}",(10,40),1,1,(255))
...
def getRandomMove(robotX,robotY,fieldX,fieldY,maxDistancePerMove) -> tuple[int,int]:
...
def addRandomRobotsMovingAround(map : probmap.ProbMap, width, height,currentRobotsAndPositions : list):
    newDetections = []
