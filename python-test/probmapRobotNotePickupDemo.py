...
def getRangeHighest(x,y):
    ...
    highest = None
    (px,py,prob) = fieldMap.getHighestGameObjectWithinRangeT(x,y,wX,wY,.30) # .30 threshold
    (objMap,robtMap) = fieldMap.getHeatMaps()
    cv2.rectangle(objMap,(int(x-wX/2),int(y-wY/2)),(int(x+wX/2),int(y+wY/2)),(255),2)
    ...
    if(highest != None and highest[0] == x and highest[1] == y and lastCollectedX != x and lastCollectedY != y):
        lastCollectedX = x
        lastCollectedY = y
        objectsCollected += 1
    ...
    while True:
        for i in range(20000):
            ...
            if(highestCoords is not None):
                ...
                Move = getMove(robotX,robotY,goalX,goalY,maxRobotSpeed*1) # 1s for now
            else:
