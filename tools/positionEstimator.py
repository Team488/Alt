...
class PositionEstimator:
    def __init__(self) -> None:
        ...
        self.__minPerc = .05 # minimum percentage of bounding box with bumper color
    ...
    def __crop_contours(self,image, combinedContour):
        ...
        cv2.drawContours(mask, [combinedContour], -1, (255,255,255), thickness=cv2.FILLED)
    ...
    def __calculateDistance(self,knownSize, currentSizePixels, cameraIntrinsics : CameraIntrinsics):
        return (knownSize * cameraIntrinsics.getFocalLength())/(cameraIntrinsics.getPixelSize() *currentSizePixels)
    ...
    def __estimateRobotHeight(self,croppedframe) -> tuple[float,bool]:
            ...
            processed,isBlue = self.__backprojCheck(labFrame,self.__redRobotHist,self.__blueRobotHist)
            if isBlue != None:
                ...
                thresholdArea = .01
                thesholdPix = int(x*y*thresholdArea*.5)
                filtered_contours = [contour for contour in contours if cv2.contourArea(contour) >= thesholdPix] 
                if filtered_contours:
                    ...
                    _,threshNumbers = cv2.threshold(backProjNumbers,35,255,cv2.THRESH_BINARY)
                    contoursNumbers, _ = cv2.findContours(threshNumbers, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                    ...
                    if contoursNumbers:
                        ...
                        #     # try to find digits
                            ...
                            if(bumperHeight == 0 or bumperWidth == 0):
                                continue
                            ratio = max(bumperWidth,bumperHeight)/min(bumperWidth,bumperHeight)
                            print("Ratio",ratio)
                        ...
                        if largestAcceptable_contour is not None:
                            ...
                            numToBumpRatio = max(height,numberHeight)/min(height,numberHeight)
                            if(numToBumpRatio < 3):
    ...
    def __estimateRelativeRobotPosition(self,frame,boundingBox,cameraIntrinsics : CameraIntrinsics) -> tuple[float,float,bool]:
        x, y, w, h = boundingBox
        midW = int(w/2)
        midH = int(h/2)
        topX = int(x-midW)
        ...
        if est is not None:
            ...
            distance = self.__calculateDistance(ObjectReferences.BUMPERHEIGHT.getMeasurement(),estimatedHeight,cameraIntrinsics)
            bearing = self.__calcBearing(cameraIntrinsics.getHFov(),cameraIntrinsics.getHres(),int(x-cameraIntrinsics.getHres()/2))
    ...
    def __estimateRelativeGameObjectPosition(self,frame,boundingBox,cameraIntrinsics : CameraIntrinsics) -> tuple[float,float]:
        x, y, w, h = boundingBox
        distance = self.__calculateDistance(ObjectReferences.NOTE.getMeasurement(),w,cameraIntrinsics)
        bearing = self.__calcBearing(cameraIntrinsics.getHFov(),cameraIntrinsics.getHres(),int(x-cameraIntrinsics.getHres()/2))
        estX = math.cos(bearing)*distance
        estY = math.sin(bearing)*distance
        return (estX,estY)
    ...
    def estimateDetectionPositions(self,frame,yoloResults,cameraIntrinsics : CameraIntrinsics):
        robotEstimatesOut = []
        ...
        for box, conf,id in zip(boxes, confs,ids):
            if(id == self.__RobotId):
                robotEst = self.__estimateRelativeRobotPosition(frame,box,cameraIntrinsics)
                if robotEst is not None:
                    robotEstimatesOut.append((robotEst,conf))
            else:
                gameObjEst = self.__estimateRelativeGameObjectPosition(frame,box,cameraIntrinsics)
