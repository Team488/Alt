class CsvParser:
    ...
    def __init__(self,csvLocation : str,minTimestepS : float,csvKeys : list[str]) -> None:
        self.keys = csvKeys
        union = self.__calculateUnion(csvKeys)
        self.values = self.__parseCsvIntoGroups(csvLocation,union,minTimestepS,csvKeys)
    ...
    def __parseCsvIntoGroups(self,csvLocation : str, union : str, minTimestepS : float,csvKeys : list[str]) -> list[list[tuple[float,str]]]:
    ...
    def __getNearestValue(self,timeSeconds : float,values : list[tuple[float,str]]) -> tuple[float,str]:
        nearest = None
        diff = 100000
        for entry in values:
            timeStamp = entry[0]
