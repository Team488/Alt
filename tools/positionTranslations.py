...
class CameraToRobotTranslator:
    ...
    def turnCameraCoordinatesIntoRobotCoordinates(self,relativeX,relativeY, cameraExtrinsics : CameraExtrinsics) -> tuple[float,float,float]:
        ...
        noteVector = np.array([[relativeX], 
                                [relativeY], 
                                [0]])  # z is 0 because we are not factoring in target height relative to camera for now
        
        # Define rotation matrices
        yawTransform = np.array([[math.cos(yaw), -math.sin(yaw), 0],
                                [math.sin(yaw), math.cos(yaw), 0],
                                [0, 0, 1]])
        
        pitchTransform = np.array([[math.cos(pitch), 0, math.sin(pitch)],
                                [0, 1, 0],
