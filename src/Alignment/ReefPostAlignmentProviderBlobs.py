...
class ReefPostAlignmentProvider(AlignmentProvider):
    ...
    def create(self):
        ...
        self.solidity = self.propertyOperator.createProperty("solidity", 0.3)
        ...
        self.ksize = self.propertyOperator.createProperty("ksize", 3)
    ...
    def align(self, frame, draw):
        ...
        for contour in contours:
            ...
            if hull_area > 0:
                solidity = area / hull_area  # Solidity close to 1 means fewer branches
                cv2.putText(
                    eroded,
                    f"{solidity:.1f}",
                    (
