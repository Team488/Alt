...
class ReefTrackingAgentBase(CameraUsingAgentBase):
    ...
    def create(self):
        super().create()
        self.tracker = ReefTracker(cameraIntrinsics=self.cameraIntrinsics,isLocalAT=True)
        self.reefProp = self.propertyOperator.createCustomReadOnlyProperty(self.OBSERVATIONPOSTFIX,b"")
