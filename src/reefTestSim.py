...
try:
    while True:
        ...
        if raw_data:
            pos = (raw_data[0]*100,MapConstants.fieldHeight.getCM()-raw_data[1]*100,-(math.radians(raw_data[2]))+math.pi) # this one gives degrees by default
            # print(f"{raw_data[2]=} {pos[2]=}")
            # pos = getPose2dFromBytes(raw_data)

            frame = np.zeros((MapConstants.fieldHeight.getCM(),MapConstants.fieldWidth.getCM(),3),dtype=np.uint8)
            drawRobotWithCams(frame,MapConstants.robotWidth.getCM(),MapConstants.robotHeight.getCM(),pos[0],pos[1],pos[2],cams)
            cv2.circle(frame,tuple(map(int,MapConstants.b_reef_center.getCM())),int(MapConstants.reefRadius.getCM()),(0,255,0),1)
            post_id = ""
            for cam in cams:
