def startDemo():
    ...
    while True:    # Create a blank image for visualization
        ...
        frame = cv2.bitwise_or(frame, cv2.merge((particleFilter.obstacles,np.zeros((frame_height,frame_width),dtype=np.uint8),np.zeros((frame_height,frame_width),dtype=np.uint8))))
        ...
        particleFilter.update_weights(observationPosition + np.random.normal(0,0.01,2),observationVelocity + np.random.normal(0,0.1,2))
