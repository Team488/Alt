...
class MultistateUkf:
    ...
    def get_nearest_valid_points(self,obstacleMap):
        ...
        nearest_valid_points = [[self.get_nearest_valid_point(i,j,obstacleMap,minDistances) for j in range(col)] for i in range(row)]
        return nearest_valid_points

    def get_nearest_valid_point(self, i, j, obstacleMap,minDistances):
        nearest = []
        heapq.heappush(nearest,(1000000,0,i, j,))
        visited = set()
        while nearest:
            ...
            if (i_n,j_n) not in visited:
                ...
                if 0 <= i_n + 1 < self.fieldX:
                    heapq.heappush(nearest, (minDistances[i_n + 1][j_n], distance_from_root + 1, i_n + 1, j_n))
                if 0 <= i_n - 1 < self.fieldX:
                    heapq.heappush(nearest, (minDistances[i_n - 1][j_n], distance_from_root + 1, i_n - 1, j_n))
                if 0 <= j_n + 1 < self.fieldY:
                    heapq.heappush(nearest, (minDistances[i_n][j_n + 1], distance_from_root + 1, i_n, j_n + 1))
                if 0 <= j_n - 1 < self.fieldY:
                    heapq.heappush(nearest, (minDistances[i_n][j_n - 1], distance_from_root + 1, i_n, j_n - 1))
    ...
    def fx(self, x, dt):
        ...
        self.baseUKF.P = eigenvectors @ np.diag(eigenvalues) @ eigenvectors.T # cov regularization 
