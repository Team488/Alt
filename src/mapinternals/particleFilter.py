...
class ParticleFilter:
    ...
    def state_transition(self, dt=1):
        ...
        pos_noise = np.random.normal(0, sum(self.field_bounds)/500, size=(self.NUM_PARTICLES, 2))
        
        accel_noise = np.random.normal(0, sum(self.field_bounds)/1400, size=(self.NUM_PARTICLES, 2))  # Slightly larger noise
    ...
    def apply_constraints(self,currentRobotHeight = 0):
        for i, particle in enumerate(self.particles):
            ...
            if x < self.field_bounds[0] or x > self.field_bounds[1]:
                self.particles[i, 0] = np.clip(x, self.field_bounds[0], self.field_bounds[1]-1)
                self.particles[i, 2] *= -1

            if y < self.field_bounds[2] or y > self.field_bounds[3]:
                self.particles[i, 1] = np.clip(y, self.field_bounds[2], self.field_bounds[3]-1)
    ...
    def update_weights(self, observationPosition, observationVelocity):
        ...
        norm_positions = self.particles[:, :2] / [self.field_bounds[1], self.field_bounds[3]]
        norm_obs_position = observationPosition / [self.field_bounds[1], self.field_bounds[3]]
