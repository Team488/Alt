...
class Window(QMainWindow):
    ...
    def onOpen(self):
        path = QFileDialog.getOpenFileName(self, "Open Image", QDir.currentPath(), "Image Files (*.png *.jpg *.bmp *.jpeg)")[0]
        if path:
            self.openImage(path)
...
class CameraWorker(QThread):
    ...
    def __init__(self, maskedAbHist=None):
        QThread.__init__(self)
        self.image_files = [os.path.join("images", f) for f in os.listdir("images") if f.endswith(('.jpg', '.jpeg', '.png'))]
        self.maskedAbHist = maskedAbHist
        self.active = True
        self.imgIndex = 0
