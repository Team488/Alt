...
from tools.Constants import ATLocations, CameraExtrinsics, CameraIntrinsics, MapConstants
from tools.Units import LengthType

class AprilTagSover:
    def __init__(self, camExtr: CameraExtrinsics, camIntr : CameraIntrinsics):
        ...
        self.reefPositioner = ReefPositioner(bluereef_center_cm=self.breefCM,redreef_center_cm=self.rreefCM)
    ...
    def __solveAprilTagTransform(self, aprilTagId,robotPose2dCmRad : tuple[float,float,float]) -> np.ndarray:
        og_robot = self.__get4x4AfflineRobotPos(robotPose2dCmRad[:2],robotPose2dCmRad[2])
    ...
    def getNearestAtPose(self, robotPose2dCmRad: tuple[float,float,float]) -> Optional[list[tuple[np.ndarray,int]]]:
        # try both blue and red reefs. If both are seen, pick the closest one
        bluepref = self.__getBluePreference(robotPose2dCmRad)
        blueRet = self.reefPositioner.getPostCoordinatesWconst(isBlueReef=True,robot_pos_cm=robotPose2dCmRad[:2],robot_yaw_rad=robotPose2dCmRad[2],camera_extr=self.camExtr,camera_intr=self.camIntr)
        redRet = self.reefPositioner.getPostCoordinatesWconst(isBlueReef=False,robot_pos_cm=robotPose2dCmRad[:2],robot_yaw_rad=robotPose2dCmRad[2],camera_extr=self.camExtr,camera_intr=self.camIntr)
