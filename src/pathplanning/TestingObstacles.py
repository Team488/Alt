...
map = probmap(fieldX, fieldY, res, objSize, objSize, robotSizeX, robotSizeY)
...
def spawnObstacles(event, x, y, flags, param) -> None:
    if event == cv2.EVENT_LBUTTONDOWN:
...
def draw_robot(image, x, y, wX, wY) -> None:
...
def draw_goal(image, x, y, radius) -> None:
...
def loop() -> None:
    while True:
        ...
        if key == ord("-"):
            ...
            path_finder_instance.update_path_with_values(
                (defaultRobotX, defaultRobotY),
                (defaultGoalX, defaultGoalY),
                obstacles_list,
                10000,
            )
