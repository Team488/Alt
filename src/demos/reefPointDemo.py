...
import cv2
import numpy as np
from tools.Constants import CameraIntrinsicsPredefined,ColorCameraExtrinsics2024,MapConstants,ATLocations
...
def visualize_transform(frame, transform, color=(0, 255, 0), length=50):
    """
...
def startDemo():
    ...
    while True:
        ...
        preferBlue = getIsBlue(robot_pos[0],b_reef_center[0],r_reef_center[0],robot_rot)
        ...
        res1 = positioner.getPostCoordinatesWconst(True,robot_pos,robot_rot,robotcam_extr,robotcam_intr)
        res2 = positioner.getPostCoordinatesWconst(False,robot_pos,robot_rot,robotcam_extr,robotcam_intr)
        results = [res1, res2]
        for res in results:
            isBlueSide=res==res1
            ...
            for postidx in postidxs:
                ...
                cv2.putText(frame,f"P Idx: {postidx}",(10,coord),1,1,(255,255,255),1)
                coord += 20
                cv2.putText(frame,f"AT Idx: {atID}",(10,coord),1,1,(255,255,255),1)
                coord += 20

                reefScreenCoordinates = UnitConversion.toint(UnitConversion.convertLength(pose[0], LengthType.IN, LengthType.CM))
                ...
                cv2.line(frame, (x0-xd,y0-yd), (x0+xd,y0+yd), (255,255,255),2)
